...
ITS_model <- function(id, tract, interruption_date, age, race, gender, name) {
  ...
  print(TS_data)
    ...
    TS_data_new <- mutate(TS_data_new, period = ifelse(Treatment == 0 & Time_since==0, "Pre",ifelse(Treatment == 1 & Time_since==0, "Event","Post")))
    TS_data_new <- data.frame(TS_data_new)

    #### CODE WORKS
    p <- ggplot(subset(TS_data_new, period != "Event"), aes(x = date, y = vaccinations_18to65, group = period)) + 
      geom_point()  + 
      ggtitle(paste("Effect of the", name, "encouragement (18 to 65)")) +
      labs(x = "Date", y  = "Daily Vaccinations (18 to 65)") + 
      geom_smooth(aes(group=period,color=period), method=lm, se=TRUE) +
      geom_vline(xintercept = TS_data_new$date[which(TS_data_new$Time == 146)], linetype = 'dashed')+
      theme_ipsum() +
      scale_fill_nejm() + 
